...
if keyboard_check(ord("A")) {
    hspeed = -5;
    sprite_index = spr_jugador_3_caminando_izquierda;
} else if keyboard_check(ord("D")) {
    hspeed = 5;
    sprite_index = spr_jugador_3_caminando_derecha;
} else {
    hspeed = 0;
...
if (sprite_index == spr_jugador_3_pelea) {
    if (keyboard_check_pressed(ord("E")) && place_meeting(x, y, obj_enemigos_2)) {
        obj_barras_vida_parte_2.vida_actual_enemigo -= 10;
...
if (keyboard_check_pressed(ord("F"))) {
    var arma = instance_place(x, y, obj_lanza_de_madera_arma);
    if (arma != noone) {
        tiene_arma = true;
        arma_instancia = arma;
        arma.x = 156;
...
with (obj_enemigos_2) {
    if (place_meeting(x, y, obj_lanzadera_de_madera_arma)) {
        ...
        instance_destroy(obj_lanzadera_de_madera_arma);
    }
}