...
if keyboard_check(ord("A")) {
    hspeed = -5;
    sprite_index = spr_jugador_2_caminando_izquierda; // Cambia al sprite de caminar hacia la izquierda
} else if keyboard_check(ord("D")) {
    hspeed = 5;
    sprite_index = spr_jugador_2_caminando_derecha; // Cambia al sprite de caminar hacia la derecha
} else {
    hspeed = 0;
...
if (sprite_index == spr_jugador_2_pelea) {
    ...
    if (keyboard_check_pressed(ord("E")) && place_meeting(x, y, obj_enemigos_parte_1)) {
        obj_barras_vida_parte_1.vida_actual_enemigo -= 10; // Reducir la vida del enemigo (ajusta según sea necesario)
...
if (keyboard_check_pressed(ord("F"))) {
    var arma = instance_place(x, y, obj_arco_arma); // Verifica si hay un arma en la ubicación del jugador
    if (arma != noone) {
...
with (obj_enemigos_parte_1) {
    if (place_meeting(x, y, obj_flecha_arma)) {
        ...
        instance_destroy(obj_flecha_arma); // Destruye la flecha después de colisionar con el enemigo
    }
}
