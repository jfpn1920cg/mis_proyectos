...
if (sprite_index == spr_jugador_7_pelea) {
    // Si está en la animación de pelea y colisiona con el enemigo al presionar la tecla E, reduce la vida del enemigo
    if (keyboard_check_pressed(ord("E")) && place_meeting(x, y, obj_enemigos_6)) {
        obj_barras_vida_parte_6.vida_actual_enemigo -= 10; // Reducir la vida del enemigo (ajusta según sea necesario)
        
        // Verifica si la vida actual del enemigo es igual o menor que cero
        if (obj_barras_vida_parte_6.vida_actual_enemigo <= 0) {
            with (obj_enemigos_6) {
