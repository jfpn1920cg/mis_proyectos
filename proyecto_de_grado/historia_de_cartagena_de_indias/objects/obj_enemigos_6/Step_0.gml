...
if instance_exists(obj_jugador_7_parte_1) {
    // Verificar la distancia entre el enemigo y el jugador
    var distance_to_player = point_distance(x, y, obj_jugador_7_parte_1.x, obj_jugador_7_parte_1.y);
    ...
    if (distance_to_player < 100) { // Ajusta el valor según sea necesario
        ...
        if (place_meeting(x, y, obj_jugador_7_parte_1)) {
            // Reducir gradualmente la vida del jugador
            obj_barras_vida_parte_6.vida_actual_jugador -= 0.1; // Puedes ajustar la cantidad de vida que se reduce

            // Verificar si la vida del jugador llegó a cero o menos
            if (obj_barras_vida_parte_6.vida_actual_jugador <= 0) {
                // Si la vida del jugador es cero o menos, destruir al jugador
                instance_destroy(obj_jugador_7_parte_1);
