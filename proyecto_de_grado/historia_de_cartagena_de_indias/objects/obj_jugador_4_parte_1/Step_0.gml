...
if keyboard_check(ord("A")) {
    hspeed = -5;
    sprite_index = spr_jugador_4_caminando_izquierda; // Cambia al sprite de caminar hacia la izquierda
} else if keyboard_check(ord("D")) {
    hspeed = 5;
    sprite_index = spr_jugador_4_caminando_derecha; // Cambia al sprite de caminar hacia la derecha
} else {
    hspeed = 0;
...
if (sprite_index == spr_jugador_4_pelea) {
    // Si está en la animación de pelea y colisiona con el enemigo al presionar la tecla E, reduce la vida del enemigo
    if (keyboard_check_pressed(ord("E")) && place_meeting(x, y, obj_enemigos_3)) {
        obj_barras_vida_parte_3.vida_actual_enemigo -= 10; // Reducir la vida del enemigo (ajusta según sea necesario)
...
if (keyboard_check_pressed(ord("F"))) {
    var arma = instance_place(x, y, obj_acha_arma); // Verifica si hay un arma en la ubicación del jugador
    if (arma != noone) {
...
if (tiene_arma && keyboard_check_pressed(ord("J"))) {
    ...
    var flecha = instance_create_depth(arma_instancia.x, arma_instancia.y, 0, obj_achasos_arma);
...
with (obj_enemigos_3) {
    if (place_meeting(x, y, obj_achasos_arma)) {
        obj_barras_vida_parte_3.vida_actual_enemigo -= 10; // Reducir la vida del enemigo (ajusta según sea necesario)
        
        // Verifica si la vida actual del enemigo es igual o menor que cero
        if (obj_barras_vida_parte_3.vida_actual_enemigo <= 0) {
            instance_destroy(); // Destruye el objeto enemigo si su vida es igual o menor que cero
        }
        
        instance_destroy(obj_achasos_arma); // Destruye la flecha después de colisionar con el enemigo
    }
}
